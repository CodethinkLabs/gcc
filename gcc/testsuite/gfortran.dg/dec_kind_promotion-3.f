...
!
      program test_small_type_promtion
        ...
        if (iabs(-9_1).ne.9) stop 2 ! { dg-error "type mismatch" }
        if (iabs(-9_2).ne.9) stop 3 ! { dg-error "type mismatch" }
        ...
        if (isign(-77_1, 1).ne.77) stop 24 ! { dg-error "type mismatch" }
        if (isign(-77_1, -1).ne.-77) stop 25 ! { dg-error "type mismatch" }
        if (isign(-77_2, 1).ne.77) stop 26 ! { dg-error "type mismatch" }
        if (isign(-77_2, -1).ne.-77) stop 27 ! { dg-error "type mismatch" }
      end program
