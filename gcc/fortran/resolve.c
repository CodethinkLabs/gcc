...
{
  gfc_warning (0, "Promoting argument for comparison from character type "
		  "to INTEGER at %L", &e->where);
  gfc_typespec t;
  t.type = BT_INTEGER;
  t.kind = 4;
  gfc_convert_type_warn (e, &t, 2, 1);
	      ...
	      if (flag_dec_non_logical_if && b->expr1->ts.type != BT_LOGICAL)
		{
		  ...
		  if (cast == NULL)
		    gfc_internal_error ("gfc_resolve_blocks(): Failed to cast "
					"to LOGICAL in IF");
		  b->expr1 = cast;
		  gfc_warning (0, "Non-LOGICAL type in IF statement condition "
			       "%L will be true if it evaluates to nonzero",
			       &b->expr1->where);
		}
